...
module.exports = (error, request, response, next) => { // eslint-disable-line
  ...
  // ================ TWILIO ERRORS ================
  if (message.includes('Please provide a text message and a proper phone number')) {
    log('info', 'Responding with a 404 status code - no text message or valid phone number');
    return response.sendStatus(404);
  }

  console.log(message);
  if (message.includes('No profile found with that phone number')) {
    log('info', 'Responding with a 404 status code - no profile found with that phone number');
    return response.sendStatus(404);
  }
